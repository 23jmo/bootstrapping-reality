  ...
  while (continueLoop) {
    try {
      ...
      const response = await anthropic.messages.create({
        ...
        } as any], // Computer Use is a beta feature with specific typing
        messages
      });
        ...
        for (const toolUse of toolUses) {
          if (toolUse.type === 'tool_use' && toolUse.name === 'computer') {
            ...
            if (action.coordinate) {
              console.log(`   Location: (${action.coordinate[0]}, ${action.coordinate[1]})`);
            }
            if (action.text) {
            ...
            // In a full implementation, this would actually execute the action
            // and return the result (e.g., screenshot data, success/failure)
            toolResults.push({
              type: 'tool_result',
              tool_use_id: toolUse.id,
              content: 'Action acknowledged. Computer Use API would execute this in production.'
            });
          }
        }
